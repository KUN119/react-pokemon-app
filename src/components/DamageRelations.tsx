...
export default function DamageRelations({ damages }: DamageModalProps) {
    ...
    const joinObjects = (props: DamageFromAndTo[], string: string) => {
        ...
        const reuslt = Object.entries(secondArrayValue)
            .reduce((acc, [keyName, value]: [string, Damage]) => {
                const key = keyName as keyof typeof firstArrayValue;c
